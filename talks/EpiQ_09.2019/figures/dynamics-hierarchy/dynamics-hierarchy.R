...
my_cols <- c("#f74700", "#b6003b", "#016394")
scales_y <- list(
...
gamma <- c(0.75, 0.25, 0.25) # partial cross-protective immunity (cpi)
...
timeseries <- julia_call("runMANTIS", strainstructure=struct, tstep=1, tmax=1000,
  ...
  mutate(variable = factor(equation, levels=c("y", "z", "w"),
         ...
         population = factor(population, levels=c("Population_1", "Population_3", "Population_2"),
                             labels=c("Population A", "Population C", "Population B"))) %>%
  as_tibble()
...
ggsave("dynamics-hierarchy.svg", width=8, height=6)
