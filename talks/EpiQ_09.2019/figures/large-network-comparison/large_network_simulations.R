...
plot_data <- disease_stats %>%
  ...
  mutate(network = factor(network, levels=rev(c("er", "sb", "tr", "ba", "ws")),
         ...
         metric = factor(metric, levels=c("ave_inf", "var_inf", "ave_res", "var_res", "ave_cycle", "var_cycle"),
                         labels=c("Mean infected (y)", "Variance in infected (y)",
                                  "Mean specific immunity (z)", "Variance in specific immunity (z)",
                                  "Mean time between epidemic peaks", "Variance in time between epidemic peaks")))
...
met1 <- ggplot(plot_data %>% filter(metric == "Mean infected (y)")) +
  aes(x=network, y=value, colour=network) +
  geom_boxplot(outlier.alpha=0, size=0.75) +
  geom_jitter(width=0.25, alpha=0.15) +
...
met3 <- ggplot(plot_data %>% filter(metric == "Mean time between epidemic peaks")) +
  ...
  geom_boxplot(outlier.alpha=0, size=0.75) +
  geom_jitter(width=0.25, alpha=0.15) +
  facet_grid("σ = 40"~metric, scales="free_y") +
  scale_colour_manual(values=my_cols) +
  scale_y_log10() +
  theme_bw() +
  theme(legend.position="none",
...
ggsave(nets, filename="summary_networks.svg", width=7, height=2)
ggsave(degs, filename="degree_distributions.svg", width=8, height=8)

(met1 + met2 + met3) * coord_flip() + plot_layout(nrow=1)
ggsave(filename="response_vars_sigma=40.svg", width=8, height=4)

## dynamical parameters
sigma <- 20               # recovery rate
...
met3 <- ggplot(plot_data %>% filter(metric == "Mean time between epidemic peaks")) +
  ...
  geom_boxplot(outlier.alpha=0, size=0.75) +
  geom_jitter(width=0.25, alpha=0.15) +
  facet_grid("σ = 20"~metric, scales="free_y") +
  scale_colour_manual(values=my_cols) +
  scale_y_log10() +
  theme_bw() +
  theme(legend.position="none",
...
(met1 + met2 + met3) * coord_flip() + plot_layout(nrow=1)
ggsave(filename="response_vars_sigma=20.svg", width=8, height=4)
